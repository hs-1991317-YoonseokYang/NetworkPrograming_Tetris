...
public class PlayerOneDraw implements IPlayerDraw {
    ...
    public void onDraw(Graphics g, int startX, int startY, int blockWidth, int blockHeight) {//startX는 게임판의 시작위치
        ...
        for (i = 0; i < tetris.getWidth(); i++) {//보드의 너비, tetris를 통해 tetrisBoard객체에 접근
            ...
            System.out.println("getWidth : "+ tetris.getWidth());
            System.out.println("startX + i * blockWidth : "+ (startX + i * blockWidth));
    ...
    private void drawRectangle(Graphics g, int x, int y, int type, int blockWidth, int blockHeight)
    {
        ...
        g.setColor(color);
        g.fillRect(x + 1, y + 1, blockWidth -2, blockHeight - 2);//여기서 창을 jpanel경계에서 띄우나 보다 ㄴㄴ<- 블럭에 음영 효과를 주기 위한 부분
